...
async function getval() {
  ...
  try {
    ...
    const userDataPromises = data.map(async (item) => {
      ...
      const responseJson = await fetch(userJsonUrl);  // Corrected: Added await
      const responseDescriptor = await fetch(`${localStorage.loginemail}.json?v=1.8&timestamp=${Date.now()}`);

      if (!responseJson.ok || !responseDescriptor.ok) {
        throw new Error(`Error fetching user data for user with ID: ${item.user_id}`);
      }

      const [json, descriptor] = await Promise.all([responseJson.json(), responseDescriptor.json()]);
      ...
      return {
        ...
        descriptor: Object.values(descriptor.descriptor),
      };
    });

    const userData = await Promise.all(userDataPromises);
    facecompare(userData);
  } catch (error) {
    console.error('Error:', error);
  }
}

async function facecompare(userData) {
  ...
  for (let i = 0; i < userData.length; i++) {
    ...
    const lbl1 = user.title;

    const s = new Float32Array(user.descriptor);
          ...
          function (position) {
            ...
            if (now >= checkinTime) {
              if (distanceToUser <= 0.3) {
                ...
                Swal.fire({
                  title: 'Confirmation',
                  text: 'Are you sure you want to check-in outside the allowed range?',
                  icon: 'warning',
