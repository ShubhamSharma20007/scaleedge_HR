...
async function facecompare() {
    ...
    if (!cachedDescriptors) {
        faceDescriptions = await faceapi.detectSingleFace(img).withFaceLandmarks().withFaceDescriptor();
        cachedDescriptors = faceDescriptions ? faceDescriptions.descriptor : null;
        localStorage.setItem(`cachedDescriptors_${loginEmail}`, cachedDescriptors);
    } else {
        faceDescriptions = { descriptor: cachedDescriptors };
    }
...
function displayTooLongMessage() {
    Swal.fire({
        icon: 'error',
        title: 'Oops!',
        text: 'Please press OK and try taking your selfie again.',
