...
window.onload = async function() {
    alert("Alert 1: Starting window onload function");
    const loginEmail = localStorage.loginemail;
    if (localStorage.getItem(`userDescriptor_${loginEmail}`)){
        await loadModels();
        alert("Alert 2: Models loaded successfully");
        faceMatcher = await createFaceMatcherfromLocalStorage();
        alert("Alert 3: Face matcher created from local storage");
    }
    else {
        await loadModels();
        alert("Alert 2: Models loaded successfully");
        faceMatcher = await createFaceMatcher();
        alert("Alert 3: Face matcher created from server data");
    }
};

// Load face recognition models
async function loadModels() {
    alert("Alert 4: Starting to load models");
    ...
    alert("Alert 5: Models loaded successfully");
}

// Create a face matcher
async function createFaceMatcher() {
    alert("Alert 6: Starting to create face matcher from server data");
    ...
    if (faceMatchDescriptor) {
        ...
        if (!userDescriptor) {
            ...
            alert("Error: Descriptor not found for logged in user.");
            return null;
        }
    ...
    alert("Alert 7: Face matcher created from server data");
...
async function createFaceMatcherfromLocalStorage() {
    alert("Alert 6: Starting to create face matcher from local storage");
    ...
    alert("Alert 7: Face matcher created from local storage");
    return new faceapi.FaceMatcher([{ label: loginEmail, descriptor: userDescriptor }]);
}

// Compare the detected face with the stored face descriptors
async function facecompare() {
    alert("Alert 8: Starting face comparison");
    try {
        ...
        if (!faceDescriptions) {
            ...
            alert("Alert 9: No face detected");
            return;
        }
        ...
        if (duration > 10) {
            ...
            alert("Alert 10: Processing time exceeded limit");
            return;
        }

        const bestMatch = faceMatcher.findBestMatch(descriptor);
        if (bestMatch._distance <= 0.51) {
            sendDataToBackend(loginEmail, duration);
            recognizedLabel = loginEmail;
            displayMatchResult(recognizedLabel, duration);
            alert("Alert 11: Face recognized successfully");

        } else {
            displayNoMatchResult();
            alert("Alert 12: No match found");
        }
    } catch (error) {
        console.error("Error in face comparison:", error);
        alert("Error in face comparison. Please try again.");
    }
}
...
function sendDataToBackend(recognizedLabel, duration) {
    alert("Sending match data to the backend");
    fetch('/storeData', {
        method: 'POST',
        headers: {
...
function displayNoMatchResult() {
    alert("Displaying no match result");
    Swal.fire({
        icon: 'error',
...
function getval() {
    alert("Fetching image from backend");
    showPleaseWait();
    fetch('/getimg', {
        method: 'POST',
        headers: {
        ...
        .then((response) => response.json())
        .then((data) => {
            alert("Image fetched successfully from backend");
            jsonObj = [];
        ...
        .catch((error) => {
            ...
            alert("Error fetching image from backend");
        });
}
...
// Trigger face comparison when file input changes
