...
async function createFaceMatcher() {
    ...
    const response = await fetch(`/megapower.json?v=${timestamp}`);

    // const response = await fetch('/megapower.json?v=2.9');
    ...
    const userDescriptor = labeledDescriptors.find(item => item.label === loginEmail)?.descriptor;
    if (!userDescriptor) {
...
// Ensure models are loaded when window is loaded
...
function displayMatchResult(label, duration) {
    Swal.fire({
        ...
        footer: `Match duration: ${duration} seconds`,
    ...
    if (isMobileDevice()) {
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function (e) {
                ...
                getval(); // Move the call to getval() inside the onload event handler
            };
