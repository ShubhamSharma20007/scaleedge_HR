...
// Load face recognition models and start initialization
...
async function loadLabeledDescriptors() {
    ...
    labeledDescriptors = json.map(data => {
...
// Function to compare the detected face with stored face descriptors
...
function compressImage(canvas) {
    ...
    var maxIterations = 10; // Maximum iterations for compression
    var minQuality = 0.1; // Minimum quality threshold
    var iterations = 0;

    while (iterations < maxIterations) {
        var compressedDataUrl = tempCanvas.toDataURL('image/jpeg', quality);
        if (compressedDataUrl.length < 10 * 1024 || quality <= minQuality) {
