...
router.get("/", function (req, res, next) {
    ...
    res.render("login", { title: "scaleedge", message: req.flash('message'), success: req.flash('success') });
});
...
function sendApprovalEmail(userData) {
    const mailOptions = {
        ...
        html: `<p>Hello ${userData.user_name},</p>
           ...
           <p>Click the following link to access your account: <a href="https://megapower.scaleedge.in/">ScaleEdge Solution</a></p>`,
    };
...
//     const customDirectory = "/home/scaleedg/megapower.scaleedge.in/public";
...
router.post("/generateusersjson", function (req, res, next) {
    ...
    fs.writeFile(
        ...
        "/home/scaleedg/megapower.scaleedge.in/public/" + userfilename,
...
router.post("/importExcel", (req, res) => {
    try {
        ...
        data.forEach((row) => {
            ...
            const employeeId = `MEGAPOWER#${nameFirstTwoLetters}`;
...
router.post('/register', async (req, res) => {
    ...
    if (enteredOTP === storedOTP) {
      ...
      const employeeId = `MEGAPOWER#${randomFourDigitNumber}`;
      ...
      const latitude = 21.1186289;
      const longitude = 79.0278583;
...
router.post("/applyArrears", function (req, res, next) {
    ...
    `;
        ...
        con.query(query, [employee_id, employee_name, amount, date, month, year, dateOfApply, remarks, Incentives, filepath, incentivesMonth, incentivesYear], function (error, results, fields) {
            if (error) {
                console.error(error);
                res.status(500).send('Internal Server Error');
            } else {
...
router.get("/arrearRequests", function (req, res, next) {
    ...
    con.query(query, function (error, results) {
        ...
        res.render("arrearRequests", { title: "scaleedge", arrearsData });
    });
});
