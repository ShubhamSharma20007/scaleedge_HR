...
router.get("/edit/:id", isAdmin, function (req, res, next) {
  ...
  var selectBranchSql = "SELECT * FROM branch_table"; // Query to fetch branches
    ...
    if (userResult.length > 0) {
      ...
      con.query(selectBranchSql, function (err, branchResult) {
        ...
        res.render("edit", {
          ...
          branches: branchResult // Pass branches data to the render function
        });
      });
    } else {
      res.redirect("/usermanagement");
...
// API endpoint to download users as Excel with specific columns and renamed headers
router.get('/downloadUsers', (req, res) => {
  const columns = [
    ...
    'reportingTo as `Reporting To`'
  ];

  const query = 'SELECT ' + columns.join(', ') + ' FROM user_master';
...
router.post('/uploadUsers', (req, res) => {
  ...
  rows.forEach(row => {
    ...
    const query = 'UPDATE user_master SET ? WHERE email = ?';
    con.query(query, [updateValues, email], (err, result) => {
      if (err) {
...
router.post("/addBranch", function (req, res, next) {
  ...
  const sql = "INSERT INTO branch_table (branch_name) VALUES (?)";
  con.query(sql, [branchName], function (err, result) {
    if (err) {
...
router.get("/addLeaveDetails", function (req, res, next) {
  const sqlCallProcedure = "CALL GetLeaveDetails()";
  con.query(sqlCallProcedure, function (err, result) {
    if (err) {
      console.error('Error calling stored procedure: ' + err.message);
      res.status(500).json({ error: 'Error calling stored procedure' });
      return;
    }
    res.render("addLeaveDetails", { title: "scaleedge", users: result[0] });
  });
});
...
router.get("/addHoursDetails", function (req, res, next) {
  const sqlCallProcedure = "CALL addHoursDetails()";
  con.query(sqlCallProcedure, function (err, result) {
    if (err) {
      console.error('Error calling stored procedure: ' + err.message);
      res.status(500).json({ error: 'Error calling stored procedure' });
    ...
    res.render("addHoursDetails", { title: "scaleedge", users: result[0] });
  });
});
